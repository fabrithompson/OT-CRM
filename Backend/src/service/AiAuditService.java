...
public class AiAuditService {
    ...
    private String buildLegacyAuditSystemPrompt(String procedures) {
        return "Sos un auditor experto y exigente de conversaciones de ventas por WhatsApp. "
                ...
                + "Antes de juzgar cada punto, revisá TODOS los mensajes del CLIENTE. "
                + "Si la información que el punto exige ya fue entregada por el cliente "
                + "(en mensajes previos, audios o documentos), el punto cuenta como "
                + "'cumplido' o 'no_aplica' — NUNCA como incumplido. NO penalices al "
                + "vendedor por 'no preguntar' algo que el cliente ya contestó. En duda "
                + "entre 'parcial' e 'incumplido', elegí 'parcial'.\n\n"
    ...
    private String buildAuditSystemPrompt(List<AuditStage> stages, AgentConfig config, Agencia agencia) {
        ...
        return "Sos un auditor experto y exigente de procesos de venta por WhatsApp. "
            ...
            + "## CONTEXTO PREVIO DEL CLIENTE — REGLA ANTI-PENALIZACIÓN\n"
            + "ANTES de juzgar una etapa, revisá TODOS los mensajes del CLIENTE en el hilo. "
            + "Si la información que la etapa exige relevar ya fue entregada explícitamente "
            + "por el cliente (sea en un mensaje previo, en una transcripción de audio o en "
            + "un documento adjunto), la etapa cuenta como 'cumplido' o 'no_aplica' — NUNCA "
            + "como incumplida.\n\n"

            + "Reglas estrictas:\n"
            + "1. Una etapa es 'incumplido' SÓLO si la información necesaria nunca apareció "
            + "en el hilo Y el vendedor tampoco la pidió. Si la info ya estaba dada por el "
            + "cliente, no hace falta que el vendedor la pregunte.\n"
            + "2. NO penalices al vendedor por 'no preguntar' algo que el cliente ya "
            + "contestó de antemano. Eso es eficiencia, no incumplimiento.\n"
            + "3. Si el cliente acelera el flujo (ej: se presenta, da necesidad y pide "
            + "precio en un solo mensaje), las etapas previas a la propuesta deben quedar "
            + "como 'cumplido' (porque la información existe en el contexto) o 'no_aplica' "
            + "(si la etapa pierde sentido en ese flujo). La justificación debe citar "
            + "textualmente el mensaje del cliente que entregó la información.\n"
            + "4. Antes de marcar 'incumplido', validá explícitamente en la 'justificacion': "
            + "\"Revisé los N mensajes del cliente y la información de [campo] no aparece en "
            + "ninguno\". Si no podés hacer esa afirmación, la etapa NO está incumplida.\n"
            + "5. En caso de duda entre 'parcial' e 'incumplido', elegí 'parcial' — "
            + "preferimos falsos negativos a acusar de incumplimiento sin evidencia.\n\n"

            + "Ejemplo correcto:\n"
            + "  Cliente: \"Hola, soy Martín, somos 4, queremos Cancún en agosto, mandame precio\".\n"
            + "  Vendedor: \"Hola Martín, soy Diego de Viajes OT. Cancún 4 pax agosto: USD 5.890\".\n"
            + "  → Presentación: cumplido (Diego se presentó).\n"
            + "  → Calificación: cumplido (el cliente entregó destino, fecha y pax — no "
            + "requiere re-preguntar).\n"
            + "  → Propuesta: cumplido (precio concreto entregado).\n"
            + "  Marcar 'Calificación: incumplido' acá sería un ERROR de auditoría.\n\n"
